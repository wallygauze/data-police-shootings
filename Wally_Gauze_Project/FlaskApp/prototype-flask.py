...
f_obs_2t3 = [t3_count['W'], t3_count['B'], t3_count['H']]  # frequencies of unarmed deceased by racial group
...
def show_tables():
    ...
    table1.rename(columns = {"sum":"count","<lambda>":"proportion"}, inplace = True)
    table2_title = "Proportion of whole respective racial group in dataset"
    table2 = df_whole[(df_whole["armed"] == "unarmed")].race.fillna(
        value="?").value_counts()/ df_whole.race.fillna(value="?").value_counts().sort_values(ascending=False)
    table2 = pd.DataFrame(table2)
    table2 = pd.DataFrame(table2.race.rename('_'))

    # :: Chi-Square test
    chisquare_result_ = chisquare(f_obs_2t3,
                                 f_exp=([x * total_positives for x in t3_group_proportions]))
    if chisquare_result_[1] < 0.01:
        chisquare_result = "Chi-Square test of independence: Significant at alpha = 1%"
    elif chisquare_result_[1] < 0.05:
        chisquare_result = "Chi-Square test of independence: Significant at alpha = 5%"
    elif chisquare_result_[1] < 0.1:
        chisquare_result = "Chi-Square test of independence: Significant at alpha = 10%"
    else:
...
@app.route("/wip") #seems like a better idea to use one function with data selection as input
def show_tables():
